/*
	F4/*		            			activate
	NUM2/8/4/6 or arrow keys			navigate thru the menus and lists (numlock must be on)
	NUM5/shift					select
...
{
	return IsKeyDown(VK_F4) || IsKeyDown(VK_MULTIPLY);
}
...
void get_button_state(bool *a, bool *b, bool *up, bool *down, bool *l, bool *r)
{
	...
	if (up) *up = IsKeyDown(VK_NUMPAD8) || IsKeyDown(VK_UP);
	if (down) *down = IsKeyDown(VK_NUMPAD2) || IsKeyDown(VK_DOWN);
	if (r) *r = IsKeyDown(VK_NUMPAD6) || IsKeyDown(VK_RIGHT);
	if (l) *l = IsKeyDown(VK_NUMPAD4) || IsKeyDown(VK_LEFT);
}
...
bool process_teleport_menu()
{
	...
	while (true)
	{
		...
		{
			// draw menu
			draw_menu_line(caption, lineWidth, 7.4, 14.0, 4.0, 4.0, false, true);
			for (int i = 0; i < lineCount; i++)
				if (i != teleportActiveLineIndex)
					draw_menu_line(lines[i].text, lineWidth, 4.0, 60.0 + i * 22.8, 4.0, 9.0, false, false);
			draw_menu_line(lines[teleportActiveLineIndex].text, lineWidth + 0.0, 2.0, 60.0 + teleportActiveLineIndex * 22.9, 4.0, 7.0, true, false);
...
void process_player_menu()
{
	...
	while (true)
	{
		...
		{
			// draw menu
			draw_menu_line(caption, lineWidth, 10.0, 13.0, 0.0, 5.0, false, true);
			for (int i = 0; i < lineCount; i++)
			...
			draw_menu_line(line_as_str(lines[activeLineIndexPlayer].text, lines[activeLineIndexPlayer].pState), 
						   lineWidth + 0.0, 2.0, 60.0 + activeLineIndexPlayer * 22.9, 4.0, 7.0, true, false);
	...
	while (true)
	{
		...
		do 
		{
			...
			draw_menu_line(line_as_str(lines[activeLineIndexWeapon].text, lines[activeLineIndexWeapon].pState), 
						   lineWidth + 0.0, 2.0, 60.0 + activeLineIndexWeapon * 22.9, 4.0, 7.0, true, false);
	...
	while (true)
	{
		...
		do 
		{
			...
			draw_menu_line(line_as_str(lines[activeLineIndexWorld].text, lines[activeLineIndexWorld].pState), 
				lineWidth + 0.0, 2.0, 60.0 + activeLineIndexWorld * 22.9, 4.0, 7.0, true, false);
	...
	while (true)
	{
		...
		do 
		{
			...
			draw_menu_line(line_as_str(lines[activeLineIndexTime].text, lines[activeLineIndexTime].pState), 
				lineWidth + 0.0, 2.0, 60.0 + activeLineIndexTime * 22.9, 4.0, 7.0, true, false);
	...
	while (true)
	{
		...
		do 
		{
			...
			draw_menu_line(line_as_str(lines[activeLineIndexWeather].text, lines[activeLineIndexWeather].pState), 
				lineWidth + 0.0, 2.0, 60.0 + activeLineIndexWeather * 22.9, 4.0, 7.0, true, false);
	...
	while (true)
	{
		...
		do 
		{
			...
			draw_menu_line(line_as_str(lines[activeLineIndexMisc].text, lines[activeLineIndexMisc].pState), 
				lineWidth + 0.0, 2.0, 60.0 + activeLineIndexMisc * 22.9, 4.0, 7.0, true, false);
		...
		if (bSelect)
		{
			...
			case 2:
				for (int i = 0; i < 1; i++) {
					GAMEPLAY::DISPLAY_ONSCREEN_KEYBOARD(TRUE, "Type anything", "", "", "", "", "", 10);
					while (GAMEPLAY::UPDATE_ONSCREEN_KEYBOARD() == 0) WAIT(0);
					char* results = GAMEPLAY::GET_ONSCREEN_KEYBOARD_RESULT();
//					char statNameFull[32];
//					sprintf_s(statNameFull, "SP%d_TOTAL_CASH", i);
//					Hash hash = GAMEPLAY::GET_HASH_KEY(statNameFull);
					set_status_text(results);
				}
				break;
			...
			case 3:
				...
				UI::SET_TEXT_COLOUR(255, 0, 0, 0);
				UI::SET_TEXT_CENTRE(20);
				UI::SET_TEXT_DROPSHADOW(0, 0, 0, 0, 0);
				UI::SET_TEXT_EDGE(0, 0, 0, 0, 0);
				UI::_SET_TEXT_ENTRY("STRING");
				UI::_ADD_TEXT_COMPONENT_STRING((LPSTR)caption.c_str());
				UI::_DRAW_TEXT(10, (((10 + 0.00278f) + 15) - 0.005f));
							//left   //top                //hegiht
		...
		do
		{
			...
			for (int i = 0; i < lineCount; i++)
				if (i != activeLineIndexVision)
					draw_menu_line(line_as_str(lines[i].text, lines[i].pState),
						lineWidth, 4.0, 60.0 + i * 22.8, 4.0, 9.0, false, false);
			draw_menu_line(line_as_str(lines[activeLineIndexVision].text, lines[activeLineIndexVision].pState),
				lineWidth + 0.0, 2.0, 60.0 + activeLineIndexVision * 22.9, 4.0, 7.0, true, false);
...
void process_money_menu()
{
	...
	{ "~b~Add~s~ 2m Money",	                      NULL,					NULL }, // case 0
	{ "~b~Add~s~ 5m Money",	                      NULL,					NULL }, // case 1
	{ "~b~Add~s~ 10m Money",	                  NULL,					NULL }, // case 2
	{ "~b~Add~s~ 15m Money",	                  NULL,					NULL }, // case 3
	{ "~r~Remove~s~ 2m Money",					  NULL,					NULL }, // case 4
	{ "~r~Remove~s~ 5m Money",					  NULL,					NULL }, // case 5
	{ "~r~Remove~s~ 10m Money",					  NULL,					NULL }, // case 6
	{ "~r~Remove~s~ 15m Money",					  NULL,					NULL }, //case 7
	{ "Add~s~ Custom Money",					  NULL,					NULL }, // case 8
	{ "Remove~s~ Custom Money",			    	  NULL,					NULL }  // case 9
	};
	...
	while (true)
	{
		...
		do
		{
			...
			draw_menu_line(line_as_str(lines[activeLineIndexMoney].text, lines[activeLineIndexMoney].pState),
				lineWidth + 0.0, 2.0, 60.0 + activeLineIndexMoney * 22.9, 4.0, 7.0, true, false);
		...
		if (bSelect)
		{
			...
			case 8:
				...
				for (int i = 0; i < 1; i++) {
					...
					sprintf_s(statNameFull, "SP%d_TOTAL_CASH", i);
					Hash hash = GAMEPLAY::GET_HASH_KEY(statNameFull);
					//						STATS::STAT_GET_INT(hash, &results, -1);
					//						STATS::STAT_SET_INT(hash, results, 1);
...
void process_main_menu()
{
	...
	std::string caption = "Ori Shamir's Mod Menu";
	...
	while (true)
	{
		...
		do 
		{
			...
			drawNotification("Made By: ~g~Ori Shamir");
